  ...
  <section className={`${styles.flexCenter} ${styles.paddingY} flex-col`}>
    <div className={`${styles.flexStart} md:flex-row flex-col mb-8 w-full`}>
      <div className=' flex-1 flex flex-col justify-start mr-10'>
        ...
        <p className={`${styles.paragraph} my-4 max-w-[310px]`}>A new way to make payments easy, reliable and secure.</p>
      </div>
    ...
    <div className='w-full flex justify-between items-center md:flex-row flex-col pt-6 border-t-[1px] border-t-[#3f3r45]'>
      ...
      <div className='flex flex-row md:mt-0 mt-6'>
          {socialMedia.map((social, index) => (
              <img key={social.name} src={social.icon} alt={social.id} className={`${index !== socialMedia.length -1 ? 'mr-6' : 'mr-0'} w-[21px] h-[21px] object-contain cursor-pointer`}/>
          ))}
      </div>
    </div>
  </section>
